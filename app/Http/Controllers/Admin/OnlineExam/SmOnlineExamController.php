...
class SmOnlineExamController extends Controller
{
    ...
    public function onlineExamReport(Request $request)
    {
        ...
        */
        if (moduleStatusCheck('OnlineExam') == true) {
            $exams = TrioOnlineExam::where('active_status', 1)
                ->where('academic_id', getAcademicId())
                ->where('school_id', Auth::user()->school_id)
                ->get(['id', 'title']);
        } else {
            $exams = SmOnlineExam::get(['id', 'title']);
        }
    ...
    public function universityOnlineExamReportSearch($request)
    {
        ...
        */
        ...
        if (moduleStatusCheck('OnlineExam') == true) {
            $online_exam_question = TrioOnlineExam::find($request->exam);
            $online_exam = TrioOnlineExam::where('class_id', $request->class)->where('section_id', $request->section)->where('id', $request->exam)->where('end_date_time', '<', $present_date_time)->where('status', 1)->first();
            $present_students = [];
            foreach ($students as $student) {
                $take_exam = TrioStudentTakeOnlineExam::where('student_id', $student->id)->where('online_exam_id', $online_exam_question->id)->first();
                if ($take_exam !== '') {
                    $present_students[] = $student->id;
                }
            }
            ...
            $exams = TrioOnlineExam::where('active_status', 1)->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->get();
        } else {
    ...
    public function onlineExamReportSearch(Request $request)
    {
        ...
        $this->validate($request, [
            ...
            if (moduleStatusCheck('OnlineExam')) {
                ...
                $exams = TrioOnlineExam::where('active_status', 1)->where('academic_id', getAcademicId())->where('school_id', $user->school_id)->get();
            } else {
                $online_exam_question = SmOnlineExam::find($request->exam);
