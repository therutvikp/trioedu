...
class SmStudentIdCardController extends Controller
{
    ...
    public function create_id_card()
    {
        ...
        */
            $user = Auth::user();
            $id_cards = SmStudentIdCard::where('active_status', 1)->where('school_id', $user->school_id)->get();
            $roles = TrioRole::select('*')->where('is_saas',0)->where('id', '!=', 1)->where(function ($q) use ($user) {
    ...
    public function edit($id)
    {
        ...
        */
            ...
            $roles = TrioRole::select('*')->where('is_saas',0)->where('id', '!=', 1)->where(function ($q) {
    ...
    public function generateIdCard()
    {
        ...
        try {
        */
            $id_cards = SmStudentIdCard::select(['id','title'])->get();
            $roles = Role::get();
            $classes = SmClass::get();
    ...
    public function generateIdCardBulk(Request $request)
    {
        ...
        if ($request->role==2) {
            if(moduleStatusCheck('University')) {
                $s_students = SmStudent::when($request->un_session_id,function($q) use ($request){ $q->whereHas('studentRecord',function($query) use ($request){
                                        $query->where('un_session_id',$request->un_session_id);
                                    });
                                })
                                ->when($request->un_faculty_id ,function($q) use ($request){ $q->whereHas('studentRecord',function($query) use ($request){
                                        $query->where('un_faculty_id',$request->un_faculty_id );
                                    });
                                })
                                ->when($request->un_department_id  ,function($q) use ($request){ $q->whereHas('studentRecord',function($query) use ($request){
                                        $query->where('un_department_id',$request->un_department_id  );
                                    });
                                })
                                ->when($request->un_semester_id   ,function($q) use ($request){ $q->whereHas('studentRecord',function($query) use ($request){
                                        $query->where('un_semester_id',$request->un_semester_id   );
                                    });
                                })
                                ->when($request->un_semester_label_id    ,function($q) use ($request){ $q->whereHas('studentRecord',function($query) use ($request){
