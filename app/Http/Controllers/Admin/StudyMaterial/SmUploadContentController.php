...
class SmUploadContentController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        */
            ...
            // if (teacherAccess()) {
            //     $teacher_info = SmStaff::where('user_id', $user->id)->with(['classes' => function ($query) {
            //         return $query->select(['id', 'class_name'])->get();
            //     }])->first();
            ...
            if (teacherAccess()) {
                $teacher_info = SmStaff::where('user_id', $user->id)
                ...
                $classes = SmClass::select('sm_classes.id', 'sm_classes.class_name')->get();
            }
            ...
            foreach ($request->available_for as $value) {
                ...
                if (($value == 'student') && ($request->status !== 'lmsStudyMaterial')) {
                    if (property_exists($request, 'all_classes') && $request->all_classes !== null) {
                        $query = StudentRecord::with('studentDetail', 'class', 'section');
                                if (shiftEnable()) {
                                    $query->with('shift');
                                }
...
/*
        ...
        */
            ...
            foreach ($request->available_for as $value) {
                if ($value == 'admin') {
                    $roles = TrioRole::where('is_saas', 0)->where('id', '=', 1)->where(function ($q): void {
                        $q->where('school_id', Auth::user()->school_id)->orWhere('type', 'System');
                    })->get();
                    foreach ($roles as $role) {
                        $staffs = SmStaff::where('role_id', $role->id)->where('school_id', Auth::user()->school_id)->get();
                ...
                if ($value == 'student') {
                    if (property_exists($request, 'all_classes') && $request->all_classes != null) {
                        ...
                        $student_ids = StudentRecord::where('class_id', $request->class)
