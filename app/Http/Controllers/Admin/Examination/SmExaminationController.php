...
class SmExaminationController extends Controller
{
        ...
        */
            if (moduleStatusCheck('University')) {
                ...
                return view('backEnd.examination.exam_attendance', compact(
                    ...
                    'un_section',
                    'subjectName',
                    'un_session_id', 'un_faculty_id', 'un_department_id', 'un_academic_id', 'un_semester_id', 'un_semester_label_id', 'un_section_id'
                ))->with($data);
                ...
                $exam_id = $request->exam;
                $subjects = SmSubject::where('active_status', 1)->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->get();
                return view('backEnd.examination.exam_attendance', compact('exam_id', 'class_id', 'section_id', 'shift_id', 'subject_id', 'exams', 'classes', 'subjects', 'exam_attendance_childs'));
            }
                ...
                if ($request->method() == 'POST') {
                   ...
                   $is_data = DB::table('sm_mark_stores')
                    ...
                    $examSubjects = SmExam::where([
                            ...
                            ->when(shiftEnable() && shiftEnable(), function ($query) use ($InputShiftId) {
                                return $query->where('shift_id', $InputShiftId);
                            })
                            ->where('school_id', Auth::user()->school_id)
                            ->where('academic_id', getAcademicId())
    ...
    public function markSheetReportStudent(Request $request)
    {
        ...
        try {
        */
            $exams = SmExamType::select(['id','title'])->where('active_status', 1)->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->get();
            $classes = SmClass::select(['id','class_name'])->where('active_status', 1)->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->get();
        ...
        */
            ...
            if (moduleStatusCheck('University')) {
                ...
                return view('backEnd.reports.mark_sheet_report_student', compact(
                    ...
                    'failgpaname',
                    'exam_type_id',
                    'section_id', 'student_id', 'exam_content', 'total_class_days', 'student_attendance', 'optional_subject_setup'));
            }
        /*
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    public function percentMarkSheetReport(PercentMarkSheetReportRequest $request)
    {
        ...
        */
            ...
            if ($exam_rule) {
                ...
                return view('backEnd.examination.report.marksheetReport', compact('exams', 'classes', 'mark_sheet', 'examInfo', 'subjectInfo', 'classInfo', 'sectionInfo','shiftInfo', 'pass_mark', 'exam_rule', 'data', 'exam_id', 'class_id', 'section_id', 'shift_id', 'subject_id'));
            } else {
                $mark_sheet = SmResultStore::query();
                $mark_sheet->where('exam_type_id', $request->exam_type);
                ...
                return view('backEnd.examination.report.marksheetReport', compact('exams', 'classes', 'mark_sheet','examInfo', 'subjectInfo', 'classInfo', 'sectionInfo', 'shiftInfo', 'pass_mark','exam_rule','data'));
