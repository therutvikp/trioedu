...
class SmClassRoutineNewController extends Controller
{
    ...
    public function classRoutinePrint($class, $section, $shift = null)
    {
        $print = request()->print;
        ...
        $sm_weekends = SmWeekend::with(['classRoutine' => function ($q) use ($class_id, $section_id, $shift_id) {
    ...
    public function dayWiseClassRoutine(Request $request)
    {
        ...
        $sm_weekends = SmWeekend::where('school_id', Auth::user()->school_id)
            ...
            ->where('active_status', 1)
            ->get();
        return view('backEnd.academics.classRoutine.form', compact('day_id', 'class_routines', 'sm_weekends', 'subjects', 'rooms', 'teachers', 'section_id', 'class_id', 'shift_id'));
    }
    ...
    public function classRoutineReportSearch(Request $request)
    {
        ...
        try {
            ...
            if(moduleStatusCheck('University')){
                ...
                $data['classes'] = $classes; //$request->shift
                $sm_routine_updates = SmClassRoutineUpdate::where('class_id', $request->class)->where('section_id', $request->section)->when(shiftEnable() && !empty($request->shift),function($query) use($request) {
