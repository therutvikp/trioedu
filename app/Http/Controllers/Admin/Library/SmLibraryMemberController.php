...
class SmLibraryMemberController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        */
            ...
            $libraryMembers = SmLibraryMember::with('roles', 'studentDetails', 'staffDetails', 'parentsDetails', 'memberTypes')
                ->where('active_status', '=', 1)
                ->where('school_id', $user->school_id)
                ->orderby('id', 'ASC')
                ->get();

            $roles = TrioRole::where('is_saas',0)->where(function ($q) use($user) {
    ...
    {
       $data = $request->all();
       $student_records = StudentRecord::with(['student','student.parents'])->where('un_session_id',$data['session_id'])
