...
<div class="erp_role_permission_area ">
    ...
    @php
        if (moduleStatusCheck('SaasRolePermission') == TRUE) {
            $module_info = Modules\RolePermission\Entities\TrioModuleInfo::where('module_id', $key)->where('active_status', 1)->first();
            $all_group_modules = Modules\RolePermission\Entities\TrioModuleInfo::where('module_id', $key)->where('id', '!=', $key)->where('active_status', 1)->get();
        } else {
            $module_info = Modules\RolePermission\Entities\TrioModuleInfo::where('parent_id',0)->where('module_id', $key)->where('is_saas',0)->where('active_status', 1)->first();
            $all_group_modules = Modules\RolePermission\Entities\TrioModuleInfo::where('module_id', $key)->where('id', '!=', $key)->where('is_saas',0)->where('active_status', 1)->get();
        }
        $check_all = 1;
        ...
        <div class="single_permission" id="{{$module_info->id}}">
            ...
            <div id="Role{{$module_info->id}}" class="collapse">
                <div  class="permission_body">
                    <ul>
                            <?php 
                                $subModule= DB::table('trio_module_infos')->where('parent_id',$module_info->id)->where('active_status', 1)->get();
                            ?>
                        ...
                        <li>
                            <div class="submodule mb-2">
                                <input id="Sub_Module_{{$row2->id}}" name="module_id[]" value="{{$row2->id}}"  class="trio_csk common-radio module_id_{{$module_info->id}} module_link"  type="checkbox" {{in_array($row2->id ,$already_assigned)? 'checked':''}}>
                                <label for="Sub_Module_{{$row2->id}}">{{$row2->name}}</label>
                                <br>
                            </div>
                            <ul class="option">
                                <?php 
                                    $childModule= DB::table('trio_module_infos')->where('parent_id',$row2->id)->where('active_status', 1)->get();
                                ?>
                                @foreach($childModule as $row3)
                                    <li>
                                        <div class="module_link_option_div" id="{{$row2->id}}">
                                            <input id="Option_{{$row3->id}}" name="module_id[]" value="{{$row3->id}}"  class="trio_csk common-radio module_id_{{$module_info->id}} module_option_{{$module_info->id}}_{{$row2->id}} module_link_option"  type="checkbox" {{in_array($row3->id ,$already_assigned)? 'checked':''}}>
