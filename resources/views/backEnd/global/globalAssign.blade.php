                                ...
                                <div class="row">
                                    @foreach ($classes as $class)
                                        <div class="col-12 mb-20 d-flex">
                                            {{ html()->form('POST', route('saveAssignedSubject'))->attributes([
                                                    'class' => 'w-100 d-flex',
                                                    'id' => 'form_' . $class->id,
                                                ])->open() }}
                                            <div class="student_rec_card">
                                                <div
                                                    ...
                                                    <button class="primary-btn small fix-gr-bg"
                                                        onclick="addMoreGlobalSubject({{ $class->id }})" type="button"
                                                        data-class_id="{{ $class->id }}"><i class="ti-plus"></i>
                                                        Add</button>

                                                    <a class="modalLink primary-btn small fix-gr-bg"
                                                        data-modal-size="modal-lg"
                                                        title="{{ $class->class_name }} @lang('study.study_material')"
                                                        href="{{ url('global-upload-content-modal?global_class_id=' . $class->id) }}">@lang('study.study_material')</a>
                                                </div>
                                                <div class="student_rec_content">
                                                        ...
                                                        <div class="col-lg-12">
                                                            <div class="primary_input">
                                                                <label class="primary_input_label"
                                                                    for="">@lang('academics.select_section')</label>
                                                                <select name="section"
                                                                    class="primary_select  form-control assignedClassSection"
                                                                    id="assignedClassSection">
                                                    ...
                                                    <div class="row mt-20">
                                                        <div class="col-lg-8"
                                                            id="classSectionWiseSubjects_{{ $class->id }}"></div>
                                                        <div class="col-lg-4"
                                                            id="classSectionWiseStudyMat_{{ $class->id }}"></div>
                                                    </div>
                                                </div>
                                                <div
                                                    class="student_rec_footer d-flex align-items-center justify-content-center">
                                                    <button class="primary-btn small fix-gr-bg saveAssignedSubject"
                                                        type="submit" data-class_id="{{ $class->id }}"
        ...
        function addMoreGlobalSubject(id) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    subject_teacher +=
                        "<option data-display='" + window.jsLang('select_subject') + "'  value=''>" + window
                        .jsLang('select_subject') + "</option>";
                    ...
                    subject_teacher +=
                        "<li data-value='' data-display='" + window.jsLang('select_subject') +
                        "' class='option selected'>" + window.jsLang('select_subject') + "</li>";
                    $.each(data[0], function(key, subject) {
                        subject_teacher +=
                            "<li data-value=" +
                            subject.id +
                    ...
                    subject_teacher +=
                        "<option data-display='" + window.jsLang('select_teacher') + "' value=''>" + window
                        .jsLang('select_teacher') + "</option>";
                    ...
                    subject_teacher += "<ul class='list'>";
                    subject_teacher +=
                        "<li data-value='' data-display='" + window.jsLang('select_teacher') +
                        "' class='option selected'>" + window.jsLang('select_teacher') + "</li>";
                    $.each(data[1], function(key, teacher) {
                        subject_teacher +=
                            "<li data-value=" +
                            teacher.id +
                    ...
                    subject_teacher += "<div class='col-lg-3 mt-30-md'>";
                    subject_teacher +=
                        "<select  class='primary_select form-control' name='exams[]'' style='width: 300px;'>"
                    subject_teacher +=
                        "<option data-display='" + window.jsLang('select_exam') + "' value=''>" + window.jsLang(
                            'select_exam') + "</option>";
                    subject_teacher += "</select>"
                    subject_teacher += "</div>";
