                ...
                <div class="row">
                    <div class="col-lg-12">
                        @if (isset($add_form_download))
                            {{ html()->form('POST', route('form-download-update'))->attributes([
                        ...
                        @else
                            @if (userPermission('form-download-store'))
                                {{ html()->form('POST', route('form-download-store'))->attributes([
                                        'class' => 'form-horizontal',
                                        'files' => true,
                                        'enctype' => 'multipart/form-data',
                                    ])->open() }}
                        ...
                        <div class="white-box">
                            ...
                            <div class="add-visitor">
                                    ...
                                    <div class="col-lg-12">
                                        <div class="primary_input">
                                            ...
                                            <textarea class="primary_input_field form-control{{ $errors->has('short_description') ? ' is-invalid' : '' }}"
                                                name="short_description" rows="3">{{ isset($add_form_download) ? @$add_form_download->short_description : old('short_description') }}</textarea>
                                ...
                                <div class="row mt-20">
                                    <div class="col-lg-12">
                                        <label class="primary_input_label" for="content_type">@lang('front_settings.content_type')</label>
                                        <select id="content_type" class="primary_select form-control" name="content_type">
                                            <option data-display="@lang('front_settings.link')" value="link"
                                                {{ old('content_type', isset($add_form_download->link) ? 'link' : (isset($add_form_download->file) ? 'file' : '')) == 'link' ? 'selected' : '' }}>
                                                @lang('front_settings.link')
                                            </option>
                                            <option data-display="@lang('front_settings.file')" value="file"
                                                {{ old('content_type', isset($add_form_download->link) ? 'link' : (isset($add_form_download->file) ? 'file' : '')) == 'file' ? 'selected' : '' }}>
                                        ...
                                        <div class="primary_input">
                                            ...
                                            <div class="primary_file_uploader">
                                                <input
                                                    ...
                                                    placeholder="{{ isset($add_form_download) ? (@$add_form_download->file != '' ? getFilePath5(@$add_form_download->file) : trans('front_settings.file') . ' *') : trans('front_settings.file') . ' *' }}"
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    @if (@$value->show_public == 1)
                                                        <button
                                                            class="primary-btn bg-success text-white border-0 small tr-bg">
...
<script>
    $(document).ready(function() {
        let initialType = '{{ old('content_type', isset($add_form_download->file) && $add_form_download->file ? 'file' : 'link') }}';

        errorShowHide(initialType);
