...
@if (isset($custom_fields) && $custom_fields)
    ...
    <div class="row mt-5">
        @foreach ($custom_fields as $key => $custom_field)
            @php
                $input_name = str_replace('-','_',Str::slug($custom_field->label));
            @endphp
            @if ($custom_field->type == 'textInput')
                <div class="{{ $custom_field->width }}">
                    <div class="primary_input ">
                        ...
                        <input
                            class="primary_input_field form-control{{ $errors->has('customF.'.$input_name) ? ' is-invalid' : '' }}"
                            type="text"
                            name="customF[{{ $input_name }}]"
                            value="{{ isset($student) ? customFieldValue($student->id, $custom_field->label, $student->custom_field_form_name) : '' }}">
                        
                        @if ($errors->has('customF.'.$input_name))
                            <span class="text-danger">
                                {{ str_replace('custom f.', '', $errors->first('customF.'.$input_name))  }}</strong>
                            </span>
                        @endif
                    </div>
                </div>
            @elseif($custom_field->type == 'numericInput')
                ...
                <div class="{{ $custom_field->width }}">
                    <div class="primary_input ">
                        ...
                        <input
                            class="primary_input_field form-control{{ $errors->has($input_name) ? ' is-invalid' : '' }}"
                ...
                <div class="{{ $custom_field->width }}">
                    <div class="primary_input ">
                        ...
                        <textarea class="primary_input_field form-control" cols="0" rows="3"
                            name="customF[{{ $input_name }}]">{{ isset($student) ? customFieldValue($student->id, $custom_field->label, $student->custom_field_form_name) : '' }}</textarea>
                    ...
                    <div class="no-gutters input-right-icon">
                        <div class="col">
                            <div class="primary_input ">
                                ...
                                <input
                                    class="primary_input_field date form-control{{ $errors->has($input_name) ? ' is-invalid' : '' }}"
                    ...
                    <div class="d-flex radio-btn-flex ml-40">
                        @if ($name_values)
                            @foreach ($name_values as $key => $name_value)
                                <div class="mr-30">
                                    <input type="radio" name="customF[{{ $input_name }}]"
            ...
            @elseif($custom_field->type == 'dropdownInput')
                ...
                <div class="{{ $custom_field->width }}">
                    <div class="primary_input ">
                        ...
                        <select class="primary_select  form-control{{ $errors->has($input_name) ? ' is-invalid' : '' }}"
                            name="customF[{{ $input_name }}]">
                            <option
                        ...
                        @if ($errors->has('customF.'.$input_name))
                            <span class="text-danger">
                                {{ str_replace('custom field', '', $errors->first('customF.'.$input_name))  }}</strong>
                            </span>
                        @endif
                    </div>
                </div>
