...
<body id="pdf">
    ...
    @php
        $body = App\SmStudentCertificate::certificateBody($certificate->body, 2,$student->student->id,$certificate->id);
        
        $body_two = App\SmStudentCertificate::certificateBody($certificate->body_two, 2,$student->student->id,$certificate->id);
        $graduate_date = null;
        if(moduleStatusCheck('Alumni') == true){
            $graduate_date = $student->alumni->graduation_date;
        }
        $certificateNumber = App\SmStudentCertificate::certificateNumber($certificate->certificate_no,$student->studentDetail->admission_no, $graduate_date);
