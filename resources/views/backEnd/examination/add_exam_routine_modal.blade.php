<div class="container-fluid">
    {{ html()->form('POST', route('add-exam-routine-store'))->attributes([
            ...
            'files' => true,
            'name' => 'myForm',
            'onsubmit' => 'return validateAddNewExamRoutine()',
        ])->open() }}
    <div class="row">
        <div class="col-lg-12">
            ...
            <input type="hidden" name="assigned_id" id="assigned_id"
                value="{{ isset($assigned_exam) ? $assigned_exam->id : '' }}">
            ...
            <div class="row no-gutters input-right-icon mt-35">
                <div class="col">
                    <div class="primary_input">
                        <input
                            class="primary_input_field  primary_input_field date form-control read-only-input has-content"
                            id="startDate" type="text" name="date" onkeyup="examRoutineCheck()"
                            value="{{ isset($assigned_exam) ? date('m/d/Y', strtotime($assigned_exam->date)) : date('m/d/Y') }}"
                            readonly="true">
                    ...
                    <select class="primary_select1 form-control" name="room" id="room">
                        ...
                        @foreach ($rooms as $room)
                            <option value="{{ @$room->id }}"
                                {{ isset($assigned_exam) ? ($assigned_exam->room_id == $room->id ? 'selected' : '') : '' }}>
                                {{ $room->room_no }} ({{ $room->capacity }})</option>
                        @endforeach
                    </select>
