...
class AddColumToTrioModuleStudentParentInfos extends Migration
{
    /**
     * Run the migrations.
     */
    ...
    {
        Schema::table('trio_module_student_parent_infos', function (Blueprint $blueprint): void {
            if (! Schema::hasColumn('trio_module_student_parent_infos', 'admin_section')) {
                $blueprint->string('admin_section')->nullable();
            }
        });

        TrioModuleStudentParentInfo::whereIn('id', [2500, 2501, 2502, 2503])->delete();
        ...
        foreach ($trio_module_student_parent_infos as $key => $value) {
            $check_exit = TrioModuleStudentParentInfo::find($value[0]);
            if ($check_exit) {
                continue;
            }
        ...
        foreach ($ids as $key => $admin_section) {
            $permission = TrioModuleStudentParentInfo::where('id', $key)->first();
            if ($permission) {
        ...
        Schema::table('sm_student_registration_fields', function (Blueprint $blueprint): void {
            if (! Schema::hasColumn($blueprint->getTable(), 'admin_section')) {
                $blueprint->string('admin_section')->nullable();
            }
        });
        ...
        if (Schema::hasTable('sm_student_fields')) {
            Schema::table('sm_student_fields', function (Blueprint $blueprint): void {
                if (! Schema::hasColumn($blueprint->getTable(), 'admin_section')) {
                    $blueprint->string('admin_section')->nullable();
                }
            });
