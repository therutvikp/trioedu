...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::table('trioedu__pages', function (Blueprint $blueprint): void {
            if (! Schema::hasColumn('trioedu__pages', 'is_default')) {
        ...
        $s2 = TrioModuleManager::where('name', 'BehaviourRecords')->first();
        if ($s2) {
            $s2->is_default = 1;
            $s2->save();
        }

        $s = TrioModuleManager::where('name', 'InAppLiveClass')->whereNull('purchase_code')->count();
        if ($s > 1) {
            $del = TrioModuleManager::where('name', 'InAppLiveClass')->whereNull('purchase_code')->first();
            $del->delete();
        }
        ...
        foreach ($filesInFolder as $fileInFolder) {
            ...
            if (file_exists($file['dirname'].'/'.$file['basename'])) {
                ...
                if ($file_data !== []) {
                    $check_exist = DB::table(config('pagebuilder.db_prefix', 'trioedu__').'pages')->where('school_id', 1)->where('slug', $file_data['slug'])->first();
                    if (! $check_exist) {
                        DB::table(config('pagebuilder.db_prefix', 'trioedu__').'pages')->insert(
                            [
                                'name' => $file_data['name'],
