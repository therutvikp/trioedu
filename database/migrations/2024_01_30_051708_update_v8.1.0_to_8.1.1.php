...
return new class extends Migration
{
    ...
    {
        $exist = TrioModuleManager::where('name', 'InAppLiveClass')->count();
        if ($exist > 1) {
            $del = TrioModuleManager::where('name', 'InAppLiveClass')->first();
            $del->delete();
        }
        ...
        foreach ($schools as $school) {
            ...
            foreach ($filesInFolder as $fileInFolder) {
                ...
                if (file_exists($file['dirname'].'/'.$file['basename'])) {
                    ...
                    if ($file_data !== []) {
                        $check_exist = DB::table(config('pagebuilder.db_prefix', 'trioedu__').'pages')->where('school_id', $school->id)->where('slug', $file_data['slug'])->first();
                        if (! $check_exist) {
        ...
        $downloadCenter = TrioModuleManager::where('name', 'DownloadCenter')->where('is_default', 0)->first();
        if ($downloadCenter) {
