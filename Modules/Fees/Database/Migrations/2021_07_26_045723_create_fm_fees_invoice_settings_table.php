...
class CreateFmFeesInvoiceSettingsTable extends Migration
{
    public function up(): void
    {
        ...
        try {
            foreach ($feesTrioModuleInfos as $feeTrioModuleInfo) {
                $checkExist = TrioModuleInfo::find($feeTrioModuleInfo[0]);
                if ($checkExist) {
                    continue;
                }

                $feesInvoiceData = new TrioModuleInfo;
                $feesInvoiceData->id = $feeTrioModuleInfo[0];
                $feesInvoiceData->module_id = $feeTrioModuleInfo[1];
                $feesInvoiceData->parent_id = $feeTrioModuleInfo[2];
                $feesInvoiceData->type = $feeTrioModuleInfo[3];
                $feesInvoiceData->is_saas = $feeTrioModuleInfo[4];
                $feesInvoiceData->name = $feeTrioModuleInfo[5];
                $feesInvoiceData->route = $feeTrioModuleInfo[6];
                $feesInvoiceData->lang_name = $feeTrioModuleInfo[7];
                $feesInvoiceData->icon_class = $feeTrioModuleInfo[8];
                $feesInvoiceData->active_status = $feeTrioModuleInfo[9];
                $feesInvoiceData->created_by = $feeTrioModuleInfo[10];
                $feesInvoiceData->updated_by = $feeTrioModuleInfo[11];
                $feesInvoiceData->school_id = $feeTrioModuleInfo[12];
                $feesInvoiceData->created_at = $feeTrioModuleInfo[13];
                $feesInvoiceData->updated_at = $feeTrioModuleInfo[14];
