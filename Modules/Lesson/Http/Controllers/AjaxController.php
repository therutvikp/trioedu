...
class AjaxController extends Controller
{
    ...
    public function getSubjectLesson(Request $request)
    {
        try {
            ...
            // return $staff_info;
            if (teacherAccess()) {
                $subject_all = SmAssignSubject::where('class_id', '=', $request->class_id)->where('teacher_id', $staff_info->id)->groupBy('subject_id')->get();

            } else {
                $subject_all = SmAssignSubject::where('class_id', '=', $request->class_id)->groupBy('subject_id')->get();
