...
class MenuManageController extends Controller
{
    ...
    {   
        ...
        if(!$role_id){
            $data['roles'] = TrioRole::where('is_saas',0)->when((generalSetting()->with_guardian !=1), function ($query) {
                $query->where('id', '!=', 3);
        ...
        $request->validate([
            'role_id' => ['required', Rule::exists('trio_roles', 'id')->where(function ($query) {
    ...
    public function store(Request $request)
    {
        try {
            ...
            foreach ($request->all_modules_id as $key => $id) {
                ...
                $sidebar->trio_module_id = $id;
                if ($user->role_id == 2 || $user->role_id == 3) {
                    $student_p = TrioModuleStudentParentInfo::find($id);
                    ...
                    $trio_module = TrioModuleInfo::find($id);
                    $sidebar->module_id = $trio_module ? $trio_module->module_id : ' ';
                    $sidebar->route = $trio_module ? $trio_module->route : ' ';
                    $sidebar->name = $trio_module ? $trio_module->name : ' ';
                    $sidebar->parent_id = $trio_module ? $trio_module->parent_id : ' ';
                    $sidebar->type = $trio_module ? $trio_module->type : ' ';
                }
    ...
    public function manage()
    {
        ...
        $role = TrioRole::where('is_saas', 0)->where('id', $id)->first();
        $all_modules = TrioModuleInfo::where('is_saas', 0)->where('active_status', 1)->get();
