        ...
        @if (Auth::user()->role_id == 2 || Auth::user()->role_id == 3)
            @foreach ($student_parent_menu as $key => $student_parent_module)
                @if (!$student_parent_module->admin_section || isMenuAllowToShow($student_parent_module->admin_section))
                    @if ($check_sidebar != '')
                        @if (in_array($student_parent_module->trio_module_id, $permission_ids))
                            <?php
                            ...
                            <div class="single_role_blocks" data-id="menu_org{{ $student_parent_module->module_id }}">
                                <div class="single_permission" id="{{ $student_parent_module->trio_module_id }}">
                                    <div class="permission_header d-flex align-items-center justify-content-between">
                                        <div>
                                            <input type="checkbox" name="module_id[]"
                                                value="{{ $student_parent_module->trio_module_id }}"
                                                id="Main_Module_{{ $key }}"
                                                class="common-radio permission-checkAll main_module_id_{{ $student_parent_module->trio_module_id }}"
                                                {{ $check_sidebar != '' ? (in_array($student_parent_module->trio_module_id, $already_assigned) ? 'checked' : '') : 'checked' }}>
                                            <input type="hidden" value="{{ $student_parent_module->trio_module_id }}"
                                                name="all_modules_id[]">
                                            <label
                                        ...
                                        <div class="permission_body">
                                            <ul class="submenuSort">
                                                @foreach ($subModule as $key => $row2)
                                                    <li>
                                                        <div class="submodule">
                                                            <input type="hidden" value="{{ $row2->trio_module_id }}"
                                                                name="all_modules_id[]">
                                                            <input id="Sub_Module_{{ $row2->trio_module_id }}"
                                                                name="module_id[]" value="{{ $row2->trio_module_id }}"
                                                                class="trio_csk common-radio  module_id_{{ $student_parent_module->trio_module_id }} module_link"
                                                                type="checkbox"
                                                    ...
                                                    <li>
                                                        <div class="submodule">
                                                            ...
                                                            <input id="Sub_Module_{{ $row2->id }}"
                                                                ...
                                                                class="trio_csk common-radio  module_id_{{ $student_parent_module->id }} module_link"
                                                                type="checkbox"
