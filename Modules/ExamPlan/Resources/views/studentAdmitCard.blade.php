        ...
        <div class="container-fluid p-0">
            ...
            <div class="row">
                <div class="col-lg-12">
                    ...
                    <div class="white-box">
                        {{ html()->form('POST', route('examplan.admitCardSearch'))->attributes(['class' => 'form-horizontal', 'files' => true, 'enctype' => 'multipart/form-data'])->open() }}
                        <div class="row">
                                        ...
                                        @if ($record->Exam)
                                            @foreach ($record->Exam->unique(['exam_type_id', 'class_id', 'section_id']) as $exam)
                                                <option value="{{ $exam->id }}"
                                                    {{ isset($exam_id) ? (@$exam->id == @$exam_id ? 'selected' : '') : '' }}>
                                                    {{ $exam->examType->title }} - {{ $record->class->class_name }}
                                                    ({{ $record->section->section_name }})</option>
                                            @endforeach
                                        @endif
                                    @endforeach
                                </select>
