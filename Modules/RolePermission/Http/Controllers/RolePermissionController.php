...
class RolePermissionController extends Controller
{
    ...
    public function role(Request $request)
    {
        try {
            ...
            $roles = TrioRole::where('is_saas',0)->when((generalSetting()->with_guardian !=1), function ($query) {
                $query->where('id', '!=', 3);
    ...
    public function assignPermission($id)
    {
        ...
        try {
           ...
           // DB::statement('SET FOREIGN_KEY_CHECKS=0;');
            $role = TrioRole::with('assignedPermission')->where('is_saas',0)->where('id',$id)->first();
...
//                dd($all_permissions);
    ...
    {
        $activeModuleList = TrioModuleInfo::whereNull('parent_route')->where('active_status', 1)->get();
    }
}