...
class BulkPrintController extends Controller
{
    ...
    public function payrollBulkPrint()
    {
        ...
        try {
            $roles = TrioRole::where('is_saas', 0)->where('active_status', '=', '1')->where('id', '!=', 1)->where('id', '!=', 2)->where('id', '!=', 3)->where('id', '!=', 10)->where(function ($q): void {
                $q->where('school_id', Auth::user()->school_id)->orWhere('type', 'System');
            })
                ->orderBy('name', 'asc')
                ->get();
    ...
    {
        try {
            $roles = TrioRole::where('is_saas', 0)->where('id', '!=', 1)->Where('type', 'System')->get();
            $classes = SmClass::get();
        ...
        try {
            ...
            if (moduleStatusCheck('University')) {
                ...
                $student_ids = StudentRecord::when($request->academic_year, function ($query) use ($request): void {
                    ...
                    ->when($request->certificateBulkClass, function ($query) use ($request): void {
            ...
            $data['roles'] = TrioRole::where('is_saas', 0)->where('id', '!=', 1)->Where('type', 'System')->get();
